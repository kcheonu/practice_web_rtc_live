...
function RestoTotalReview({ restaurant_id }) {
    const validRestaurantId = restaurant_id || "1"; // ✅ 기본값 설정
    ...
    console.log("현재 페이지의 restaurantId:", validRestaurantId);

    // ✅ 가게 리뷰 데이터 불러오기
    useEffect(() => {
        if (!validRestaurantId) {
            console.error("❌ restaurantId가 undefined입니다. API 요청을 스킵합니다.");
            return;
        }
        ...
        const fetchReviews = async () => {
            try {
                ...
                if (!response.ok) {
                    throw new Error(`❌ 리뷰 데이터를 가져오지 못했습니다: ${response.status}`);
                }
                ...
                console.error("❌ 리뷰 데이터 가져오는 중 오류 발생:", err);
    ...
    return (
        <div className="p-4 bg-white">
            {/* ✅ 가게 정보 */}
            <RestoValue restaurantId={validRestaurantId} />

            {/* ✅ 리뷰 카운트 및 필터 */}
            <div className="flex items-center justify-between mb-4">
                <div className="text-lg font-semibold">리뷰 {reviews.length}개</div>
